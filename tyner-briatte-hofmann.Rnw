...
\begin{figure}[hbtp]
\centering
<<madmen_ex_ggnet, dependson='reading_data', out.width='\\textwidth', fig.width=9, fig.height=9.5>>=
...
In this section, we demonstrate the current capabilities of \samp{ggnet2}, {geom\_net} and \code{ggnetwork} in a series of side by side examples.
...
ggplot(data = blood$edges, aes(from_id = from, to_id = to)) +
  geom_net(layout = "circle", size = 15, colour = "darkred", 
           label = TRUE, labelcolour = "grey80",
           directed = TRUE, linewidth = 0.75, arrowsize = 1.5, arrowgap = 0.05) +
  theme_net()
@
...
\caption{\label{fig.cap:theme_ex} Inheritance structure of \pkg{ggplot2} theme elements. This is a recreation of the graph found at \protect\url{http://docs.ggplot2.org/current/theme.html}.}
\end{figure}
\afterpage{\clearpage}
