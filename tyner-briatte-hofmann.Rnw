...
@
\includegraphics[width=.75\textwidth]{figure/compare-1.pdf}
\caption{\label{fig:timings} Comparison of the times needed for calculating and rendering the protein interaction network in our three \pkg{ggplot2} approaches and the standard plotting routine of the \pkg{network} and \pkg{sna} packages based on 100 evaluations each.}
\end{figure}
...
% Further testing on a network of [EDGES] interactions between [NODES] proteins yielded the same conclusion. [INSERT protein-based test here.]
=======
\hh{XXX how does edge probability affect these results?
