...
Other arguments passed to the \code{ggnet2} function offers extensive control over the aesthetics of the plot that it returns, including %through
...
Fortunately, R provides functionality that allows for an easy way of producing the required result: \code{merge} (in base R), \code{join} (in \pkg{plyr}) and \code{full\_join} (in \pkg{dplyr}) can be used. In \code{merge} the argument \code{all} needs to be set to \code{TRUE}; in \code{join}, the parameters have to be set to \code{type = "full"} and \code{all = TRUE}. Both operations are equivalent to a `full join' in SQL terminology.
...
ggplot(data = emailnet,
  ...
  geom_net(
    ...
    ealpha = 0.2, size = 4, curvature = 0.05,
...
Comparing the results of \code{ggnet2} \code{geomnet} and \code{ggnetwork} reveals some of the subtle differences that exist between each implementation:

\begin{itemize}

  \item %
  In the \code{ggnet2} implementation, the darkness and thickness of the edges between employees in the same cluster are more promounced than those of the edges between employees in different clusters. This is due to the fact that the email network does not feature edge weights: instead, every email between two employees is represented by a different edge, resulting in edge overplotting. Since the \code{edge.alpha} argument has been set to a value smaller than 1, multiple emails between two employees create darker, thicker edges between them. This effect is not visible in the layouts created by \code{geom\_net} and \code{ggnetwork} because both implementations draw a single segment between connected nodes, regardless of how many edges exist between them.
  ...
  In the first two layouts, edges between employees who share the same employment type are given the color of that employment type, while edges between employees belonging to different types are plotted in grey. This feature is particularly useful to visualize the amount of within-group connectedness in a network. By contrast, in the last layout, edges are colored according to the sender's employment type, because the \code{ggnetwork} package does not support coloring edges as a function of node-level attributes.
	...
	\item %
	  Last, in the last two layouts, the \code{curvature} argument has been set to 0.05, resulting in slightly curved edges in both plots. This feature, which takes advantage of the \code{geom\_curve} geometry released in \pkg{ggplot2} 1.1.0, makes it possible to visualize which edges correspond to reciprocal connections; in an email communication network, as one might expect, most edges fall into that category.

\end{itemize}

Last, it is interesting to observe that, in all of the layouts, the `Security' employee group gets a central role in the email network of the company. This observation holds regardless of the actual network layout used (remember that \code{ggnet2} and \code{ggnetwork} default to the Fruchterman-Reingold algorithm, while \code{geomnet} defaults to the Kamada-Kawai algorithm; both layouts are force-directed, but proceed differently to assign coordinates to the nodes). Interestingly, one of the executives only communicates with a single person in Facilities. Last, one of the IT employees is also frequently communicates with a Security employee.
...
\afterpage{\clearpage}
%
Figure~\ref{fig:email_facet} shows the facetted network. Through the facetting it becomes obvious that there are several days where one or more of the departments does not communicate with any of the other departments. There are only two days, 13 and 15, without any isolated department communications.
