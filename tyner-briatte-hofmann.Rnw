...
ggplot(data = emailnet, aes(from_id = From, to_id = to)) +
  geom_net(aes(colour = CurrentEmploymentType, 
               group = CurrentEmploymentType), fiteach = TRUE, 
           linewidth = 0.5, ealpha = 0.5, size = 1.5, directed = TRUE, 
           arrowsize = .3) +
...
\begin{figure}[hbtp]
\caption{Source code for visualizing the Mad Men network using geom\_net }
