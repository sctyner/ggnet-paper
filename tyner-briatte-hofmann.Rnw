...
\sct{XXX Fran\c{c}ois, I agree. The original intent was to demonstrate the ability of the geom to change edge widths based on some weight variable. But, this is also done now in the bike share example, which also happens to be a much cooler and more unique example. I have commented the rest of the Les Mis section out for now. XXX}
...
The second test relies on random undirected networks in which the probability of an edge to exist between two nodes was set to 0.2. 
...
We generated 100 of these networks at different network sizes between 20 and 240 nodes, using increments of 20. We then computed the average plotting time at each network size, as shown in Figure~\ref{fig.cap:runtimes-random}.
