...
We generated 100 of these networks at network sizes from 25 to 250 nodes, using increments of 25. %We then computed the average plotting time at each network size, as shown in Figure~\ref{fig.cap:runtimes-random}.
...
\footnote{\hh{The code used for benchmarking can be found at \url{https://raw.githubusercontent.com/sctyner/ggnet-paper/master/runtimes/run.r}. Run time results are found in \url{https://github.com/sctyner/ggnet-paper/tree/master/runtimes}.}}
...
<<runtimes-all, echo=FALSE, fig.width=8, fig.height=6>>=
library(tidyr)
library(reshape2)
...
qplot(data = timings, reorder(type, elapsed, median), elapsed,
      fill = type, alpha = I(0.6), geom = "boxplot") +
  scale_fill_brewer(palette = "Set2") +
