...
\item in the intro: emphasize the goal of the paper: visualizations of networks within the ggplot2 framework and nothing but visualizations.
\item cite \CRANpkg{ndtv} package by \cite{ndtv} - this has some fancy animations of  dynamic temporal networks -- should probably go into the email example
\end{itemize}
XXX}
...
This network is plotted in Figure~\ref{fig.cap:email}. \sct{In each example, we have colored the vertices by the variable \code{CurrentEmploymentType}, which contains the department in the company of which each employee is a par.} There are six distinct clusters in this network which almost perfectly correspond to the six different types of employees in this company: administration, engineering, executive, facilities, information technology, and security.
...
\hh{In a two-mode or bipartite network two classes of nodes exist and edges only connect nodes from different classes. 
