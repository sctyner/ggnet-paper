...
%
and \sct{then consider} average plotting times of increasingly larger random networks.%
...
@
\hh{XXX and again, we have a qualitative difference to the previous results. Where does that come from? The only option that I see is the edge probability - which for the protein network is \Sexpr{round(nrow(protein$edges)/choose(length(protein$vertices),2), 3)} (4480 out of \Sexpr{round(choose(length(protein$vertices),2),3)} theoretically possible connections).
}
