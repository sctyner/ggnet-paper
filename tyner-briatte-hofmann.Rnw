...
\abstract{
\sct{This paper explores three new functionalities built on the \CRANpkg{ggplot2} framework: \texttt{geom_net}, \texttt{ggnet2}, and \texttt{ggnetwork}.  Together, these additions }
}
...
Many R packages already exist for network analysis and visualization such as \CRANpkg{igraph} by \citet{igraph}, \CRANpkg{sna} by \citet{sna}, and \CRANpkg{network} by \citet{network.jss, network}.
\hh{Unfortunately, there is not a standard format for network results, which requires from  users detailed knowledge of each one of these packages to be able to make meaningful changes to the network, such as}
...
facilitating the extension of \pkg{ggplot2} in form of developing additional geoms,
which is exactly
