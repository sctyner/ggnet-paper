...
detailed knowledge of each one of these packages \sct{and their syntax} in order to build meaningful network visualizations with them. 
...
\sct{Thus, our focus here is not on adding to the important theory of graph drawing, \citep[cf.\ ][]{graphdrawing} but rather the implementation of current graph drawing capabilities in the \CRANpkg{ggplot2} framework.} \sct{The \CRANpkg{ggplot2} package} was designed as an implementation of the `grammar of graphics' proposed by \citet{wilkinson:1999}, and it has become extremely popular among R users.%
  %
...
%
\item {\bf edges:} for edges we distinguish between two different sets of aesthetics: aesthetics that only relate to line attributes, such as  \code{linewidth} and \code{line type}, \sct{and aesthetics that are also used by the point \code{geom}}.  \sct{The former} can be used in the \sct{same way as they are used in \code{geom\_segment}, %Additionally, node aesthetics
while the latter, like} \code{alpha} or \code{colour}, \sct{for instance}, are used for vertices unless separately specified. \sct{Instead, use} the parameters \code{ecolour} or \code{ealpha}, which are only applied to the edges. If the \code{group} variable is specified, a new variable, called \code{samegroup} is added during the layout process. This variable is \code{TRUE}, if an edge is between two vertices of the same group, and \code{FALSE} otherwise. If \code{samegroup} is \code{TRUE}, the corresponding edge will be colored using the same color as the vertices it connects. If the edge is between vertices of a different group, \sct{the} default grey shade is used for the edge.
...
@
\end{subfigure}
\caption{\label{fig:davis}(continued) Graph of the Southern women data. Women are represented as orange triangles, events as green circles. }
\end{figure}
%
\noindent
