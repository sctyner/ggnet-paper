...
In this section, we demonstrate the current capabilities of \code{ggnet2}, \code{geom\_net} and \code{ggnetwork} in a series of side by side examples. \sct{While the output is similar for each method of network visualization, the code and implementations differ across the three methods.
