...
The packages described in this paper have, by contrast, have one primary purpose: to create beautiful network visualizations by providing a wrapper of existing network layout capabilities (see for example the \CRANpkg{statnet} suite of packages by \citet{statnet}) to the popular \CRANpkg{ggplot2} package \citep{ggplot2}.
%
