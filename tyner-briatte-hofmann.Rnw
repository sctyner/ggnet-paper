...
\abstract{
\sct{This paper explores three new functionalities built on the \pkg{ggplot2} framework: \texttt{geom_net}, \texttt{ggnet2}, and \texttt{ggnetwork}.  We will show that together, these additions contribute significantly to the visualization of networks. }
}
...
\hh{Unfortunately, there is not a standard format for network results, which requires fromusers detailed knowledge of each one of these packages to be able to make meaningful changes to the network, such as}
...
We chose to fill this gap by adding network plotting capabilities to the popular and widely used R package \pkg{ggplot2}. Just to give an idea of the popularity and the wide-spread use of \pkg{ggplot2}, from January 1, 2015 to October 10, 2015, \pkg{ggplot2} was downloaded over 1.2 Million times \sct{from CRAN: more than 4,000 per day.} It has also been downloaded in 215 countries at least once, and in 21 of those countries, including China, Israel, and Colombia, it has been downloaded over 10,000 times\footnote{\pkg{ggplot2} usage statistics taken from \url{http://cran-logs.rstudio.com/}.}. This is the user base we are aiming at by making network visualizations a part of \pkg{ggplot2}.

\hh{Besides the wide-spread use of \pkg{ggplot2}, its design based on the Grammar of Graphics \citep{wilkinson:1999} makes it relatively easy to extend. Many extensions already exist:}
\fb{
...
\sct{mapping via the \pkg{ggmap} package by \citet{ggmap}, visualization of statistical models via the \pkg{ggfortify} package by \citet{fortify}, many helper functions in the \pkg{GGally} by \citep{ggally}, and \textit{Bioconductor} plotting via the \pkg{ggbio} by \citep{ggbio}, to name a few. Just as these examples have increased the utility of \pkg{ggplot2}, our additions aim to extend the benefits of the grammar of graphics to network visualization.}
...
\subsection{ggnet2}
\fb{ggnet2 is the most 'hacky' of the three implementations presented in this paper. The function takes a network object as input, and then recreates every necessary step internally to convert that object for use with ggplot2. This happens through the creation of two data frames, one for edges, another one for nodes, which are then passed to ggplot2. All steps happen internally; as a consequence, while it uses the grammar of graphics to produce the plot, the function still works very much like the plotting functions of the igraph and network packages, i.e. through a long series of arguments that control either the aspect of the edges or the aspect of the nodes.}
...
%% NOTE: the code used to create a "sequential data frame" might become an
%% option to ggnetwork in the ggnetwork package
...
# data step 1: create a sequence of networks
em.day <- subset(email$edges, nrecipients < 54)[, c("From", "to", "day") ]
em.day <- lapply(unique(em.day$day), function(x) subset(em.day, day == x)[, 1:2 ])
em.day <- lapply(em.day, network, directed = TRUE)
for (i in 1:length(em.day)) {
  em.day[[ i ]] %v% "curr_empl_type" <- em.cet[ network.vertex.names(em.day[[ i ]]) ]
  em.day[[ i ]] %n% "t" <- unique(email$edges$day)[ i ]
}
# data step 2: convert sequence to a data frame
em.day <- lapply(em.day, function(x) {
  df <- ggnetwork(x, arrow.gap = 0.02)
  df$day <- x %n% "t"
  df
})
em.day <- dplyr::bind_rows(em.day)

# create the plot
ggplot(em.day, aes(x, y, xend = xend, yend = yend)) +
    geom_edges(color = "grey50") +
    geom_nodes(aes(color = curr_empl_type)) +
    facet_wrap(~ day) +
  scale_colour_brewer("Current employment type", palette = "Set1") +
  theme_blank() +
  facet_wrap(~ day, nrow = 2, labeller = "label_both") +
  theme(legend.position = "bottom")
@
...
\begin{subfigure}[t]{\textwidth}
\caption{ggnetwork}
<<madmen2_ggnetwork, size="footnotesize", opts.label="codefig", echo=TRUE>>=
ggplot(
  ggnetwork(mm.dir,
            layout = "fruchtermanreingold"),
       aes(x, y, xend = xend, yend = yend)) +
