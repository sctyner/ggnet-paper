...
library(sna)
@
% From reviewer 1: "There is a niche in computer
% science called "graph drawing", which has been around for decades and
% which is concerned with layout algorithms for networks. This is import
% because it is all but a new insight that network layouts are important
% and there is even a whole scientific specialty that is concerned with
% this. Yet the authors do not engage with this literature at all. They
% should mention "graph drawing" and refer to some of the classic
% works. A good starting point is the 2001 Springer book by Kaufmann and
% Wagner with the title "Drawing Graphs", or the table of contents of
% the "Journal of Graph Algorithms and Applications", or the book
% "Layout of Graph Visualizations" by Ulrik Brandes (1999), along with
% the many innovations in graph drawing produced since then by people
% like Ulrik Brandes and others. All of this seems to be ignored by the
% authors."
% \sct{I've come around to the idea of mentioning something about this in passing. i.e. we could mention that our purpose was not to engage with the theory of graph drawing but rather to engage with the current visualization methods to make them easier to access by users}

\hh{XXX Still todo: 
\begin{itemize}
\item mention weights in networks. Thresholding of weights is then a subsetting routine - could maybe go into the conclusion? 
%\item in the intro: emphasize the goal of the paper: visualizations of networks within the ggplot2 framework and nothing but visualizations. DONE
%\item cite \CRANpkg{ndtv} package by \cite{ndtv} - this has some fancy animations of  dynamic temporal networks -- should probably go into the email example XXX
\end{itemize}
XXX}
...
Many R packages already exist to manipulate network objects, such as \CRANpkg{igraph} by \citet{igraph}, \CRANpkg{sna} by \citet{sna}, and \CRANpkg{network} by \citet{network} (see also \citet{network.jss}). \sct{Though t}hese packages \sct{have network visualization capabilities, visualization is not their primary purpose. This leads to visualization techniques in that require } %however, rely on idiosyncratic visualization syntaxes, which means that the user must acquire 
detailed knowledge of each one of these packages \sct{and their syntaxes} in order to build meaningful network visualizations with them. \sct{XXX I'm not sure what you're trying to say in this next sentence, Francois. Could you explain a bit in a comment? XXX} These packages further require to repeat all graphic parameters (such as colors, coordinates and sizes) in every network visualization, with no possibility to use precomputed layers of these parameters, or to map these parameters to network attributes. \sct{To contrast, the packages we have written have one primary purpose: to provide a means of creating beautiful network visualizations within a framework that is accessible to and used by many.}

% solution: ggplot2 as an implementation framework
%\sct{This is redundant} In this article, 
Our objective is to bring network plotting capabilities to the popular \CRANpkg{ggplot2} package \citep{ggplot2}. \sct{Thus, our focus here is not on adding to the important theory of graph drawing, (cf. \citet{graphdrawing}) but rather the implementation of current graph drawing capabilities in the elegant \CRANpkg{ggplot2} famework.} \sct{The \CRANpkg{ggplot2} package} was designed as an implementation of the `grammar of graphics' proposed by \citet{wilkinson:1999}, and it has become extremely popular among R users.%
  %
...
\sct{\section{Brief introduction to networks}}
\label{sec:background}
% XXX SCT-Making the language more precise, per reviewer 1. 
In its essence, a network is simply a set of %points
\sct{actors} connected in pairs by a set of %lines
\sct{relationships} \citep{newman}. %Here, we refer to the lines as edges (or ties) and to the points as nodes (or vertices).
\sct{Here, we refer to the actors and relationships by their graph representation equivalents, vertices (or nodes) and edges, respectively.} We note that our examples of visualization are all of one-mode networks, where all nodes are of the same kind and are able to connect to each other, however the graphical tools presented can be used to visualize %more general 
\sct{many other} types of networks. %\sct{XXX I'm not 100\% sure that's true but it seems right.XXX
% HH: more general is definitely true}

The two sets of graphical objects that make up a network \sct{visualization}, points and segments between them, have been used to %encode 
\sct{examine} a huge variety and quantity of information across many different fields of study. For instance, networks of scientific collaboration, a food web of marine animals, and American college football games are all covered in a paper on community detection in networks by \citet{football}. \citet{networkfailures} %examine 
\sct{study} node failure in interdependent networks like power grids.  Social networks such as links between actors found on \url{http://www.imdb.com/} and neural networks, like the completely mapped neural network of the \textit{C. elegans} worm are also extensively studied \citep{smallworld}.

Networks can vary widely in scope and complexity: the smallest network is simply %an edge 
\sct{one tie} between two %vertices,
\sct{actors}, while one of the most commonly used and most complex networks, the world wide web, has billions of vertices (Web pages) and billions of edges (hyperlinks) connecting them. Additionally, the edges in a network can be directed or undirected: directed edges represent information travelling from one vertex to another, and switching the direction would change the structure of the network. The World Wide Web is an example of a directed network because hyperlinks connect one Web page to another, but not necessarily the other way around. Undirected edges are simply connections between vertices. Co-authorship networks are examples of undirected networks, where nodes are authors and they are connected by an edge if they have written an academic publication together.

%An even more common network example is a social network. 
% Rewrite the hated sentence!
\sct{As a reference example, we turn to a specific instance of a social network.} A social network is a network that everyone is a part of in one way or another, \sct{whether through friends, family, or other human interactions}. We do not necessarily refer here to social media like Facebook or LinkedIn, but rather to the connections we form with other people. To demonstrate the functionality of our tools for plotting networks, we have chosen an example of a social network from the popular television show \emph{Mad Men}. This network, which was compiled by \citet{madmen} \hh{and made available in \CRANpkg{gcookbook} \citep{gcookbook}}, is made up of 52 vertices and 87 edges. Each vertex represents a character on the show, and there is an edge between every two characters who have had a romantic relationship.
...
The package \pkg{geomnet} implements network visualization in a single \pkg{ggplot2} layer. \sct{A stable version is available on CRAN, with a development version available at} \url{https://github.com/sctyner/geomnet}. The package has two main functions:  \code{stat\_net}, which performs all of the calculations, and \code{geom\_net}, which renders the plot. \sct{It also contains the secondary functions \code{geom\_circle} and \code{theme\_net}, which assist, respectively, in drawing self-referencing edges and removing axes and other background elements from the plots.}
...
\sct{In \CRANpkg{sna}, for each layout there is a corresponding set of possible layout parameters, \code{layout.par}, which can be passed to \code{geom\_net}. If the user wishes to create small multiples using \CRANpkg{ggplot2} \code{facets}, they can use }\code{fiteach}, a logical value specifying whether each panel's data should be fit separately (default) or whether the same layout should be used for all of the \sct{small multiples}.
\item {\bf vertices:} any of \pkg{ggplot2}'s aesthetics relating to points: \code{colour}, \code{size}, \code{shape}, \code{alpha}, \sct{and \code{stroke}} are available and used for specifying the appearance of nodes in the network.
%
\item {\bf edges:} for edges we distinguish between two different sets of aesthetics: aesthetics that only relate to line attributes, such as  \code{linewidth} and \code{linetype}, \sct{and aesthetics that are also used by the point \code{geom}}.  \sct{The former} can be used in the \sct{same way as they are used in \code{geom\_segment}, %Additionally, node aesthetics
while the latter, like} \code{alpha} or \code{colour}, \sct{for instance}, are used for vertices unless separately spcified. \sct{Instead, use} the parameters \code{ecolour} or \code{ealpha}, which are only applied to the edges. If the \code{group} variable is specified, a new variable, called \code{samegroup} is added during the layout process. This variable is \code{TRUE}, if an edge is between two vertices of the same group, and \code{FALSE} otherwise. If \code{samegroup} is \code{TRUE}, the corresponding edge will be colored using the same color as the vertices it connects. If the edge is between vertices of a different group, \sct{the} default grey shade is used for the edge.
...
{\bf self-referencing vertices:} some networks contain self references, i.e.\  an edge has the same vertex id in its from and to columns. If the parameter \code{selfies} is set to \code{TRUE}, a circle is drawn \sct{using the new \code{geom\_circle}} next to the vertex to represent this self reference.

%
\item {\bf arrow:} whenever the parameter \code{directed} is set from its default state to \code{TRUE}, arrows are drawn from the `from' to the `to' node, with tips pointing towards the `to' node.  By default, arrows have an absolute size of 10 points. \sct{The entire structure of the arrow can be changed by passing an \code{arrow} object from the \CRANpkg{grid} package to the \code{arrow} argument. If the user doesn't wish to change the whole arrow object, the parameters \code{arrowsize} and \code{arrowgap} are also available.} The \code{arrowsize} argument is of a positive numeric value that is used as a multiple of the original arrow size, i.e.\ \code{arrowsize = 2} shows arrow tips at twice their original size.  The parameter \code{arrowgap} can be used to avoid overplotting of the arrow tips by the nodes, \code{arrowgap} specifies a proportion by which the edge should be shrunk with default of 0.05. A value of 0.5 will result in edges drawn only half way from the `from' node to the `to' node.
\item {\bf labels:}  \code{label} can be used as either a logical parameter or as a data variable, in which case it is assumed that the associated variable consists of the character strings to be used for labeling the nodes. Again, if \code{colour} is specified for the nodes, the same values are used for the labels, unless \code{ labelcolour} is specified. \sct{If \code{fontsize} is specified, it changes the label size to that value in points.} Other parameter values, such as \code{vjust} and \code{hjust} help in adjusting labels relative to the nodes. The parameters work in the same fashion as in native \pkg{ggplot2} geoms. \sct{Additionally, the label can be drawn by using \code{geom\_text} (the default) or using the new \code{geom\_label} in \CRANpkg{ggplot2} by adding \samp{labelgeom = "label"}} to the arguments in \code{geom\_net}.  
...
In this section, we demonstrate \sct{some of XXX to appease the reviewers want for an example of everything XXX } the current capabilities of \code{ggnet2}, \code{geomnet}, and \code{ggnetwork} in a series of side by side examples. While the output is %similar
\sct{nearly identical} for each method of network visualization, the code and implementations differ across the three methods.
For each of these examples, we are going to present the code necessary to produce the network \sct{visualization in each of the three packages}, and discuss each application in detail.

\subsection{Blood donation}  % =================================================

\sct{We begin with a very simple example that most should be familiar with: blood donation.} In this directed network, there are eight vertices and 27 edges.  The vertices represent the eight different blood types in humans that are most important for donation: the ABO blood types A, B, AB, and O, combined with the RhD positive (+) and negative (-) types. The edges are directed: a person whose blood type is that of a \emph{from} vertex can to donate blood to a person whose blood type is that of a corresponding \emph{to} vertex.  This network is shown in Figure~\ref{fig.cap:blood}. The code to produce each one of the networks is shown below:
...
The inheritance structure is plotted in figure \ref{fig.cap:theme}.  In these plots, it is easy to quickly determine the parent and child vertices, \sct{and to view which theme elements are unrelated to all others.} The parents with the most children are the \code{rect}, \code{text}, and \code{line} elements, so we made their labels larger in order to emphasize their importance. In each case, the label size is a function of the out degree of the vertices.
...
\hh{One of the classic examples for a two-mode network is the network of 18 Southern women attending 14 social events as collected by \citet{davis} and published e.g.\ as part of the \CRANpkg{tnet} package \citep{tnet}, i.e.\ a woman is linked by an edge to an event if she attended it. 
...
@
\noindent
\hh{The edge list for the Southern women's data consists of women attending events:}
<<data_davis_list, dependson='davis_data_1', echo=TRUE>>=
head(elist)
@
\noindent
\hh{In order to distinguish between nodes from different types, we have to add an additional identifier element, so that we can distinguish between woman $L1$ and the first event, $E1$.}
...
ggplot(data = davis) + 
  ...
  theme(legend.position = "bottom")
@
\caption{\label{fig:davis:geomnet}Graph of the Southern women data using a Kamada-Kawai layout in \pkg{geomnet}. Women are represented as orange triangles, events as green circles. }
\end{figure}
...
Figure~\ref{fig:bikes} shows four renderings of this data. The first is a geographically true representation of the area overlaid by lines between bike stations, the other three are networks drawn with \pkg{geomnet}, \pkg{ggnet2}, and \pkg{ggnetwork}, respectively. \sct{To plot the geographically correct bike network layout in \CRANpkg{geomnet}, we use the \samp{layout = NULL} option and provided the latitude and longitude coordinates of the bike stations from the company's data. In \pkg{ggnetwork} and \code{ggnet2}, XXX Francois, explain how to do it in these packages to appease reviewer 1's comments about the custom layoutsXXX }
...
For the geographic representations we also made use of the three packages. Because all three approaches result in the same picture, we only show one of these in figure~\ref{fig:bikes}a. The code \sct{for creating the map} is given here:
...
%' map_01 <- metro_map
%' attr(map_01, "bb") <- data.frame(ll.lat=0, ll.lon=1, ur.lat=0, ur.lon=1)
%' 
...
\hh{The code to create benchmark results for both of these situations is provided in the online supplement.} 
