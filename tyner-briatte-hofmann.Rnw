...
Many R packages already exist to manipulate network objects, such as \CRANpkg{igraph} by \citet{igraph}, \CRANpkg{sna} by \citet{sna}, and \CRANpkg{network} by \citet{network} (see also \citet{network.jss}). Though these packages have network visualization capabilities, visualization is not their primary purpose. This leads to visualization techniques in that require  %however, rely on idiosyncratic visualization syntaxes, which means that the user must acquire 
detailed knowledge of each one of these packages and their syntax in order to build meaningful network visualizations with them. 
...
%How about: XXX  }
All of these packages are developed with a focus of analyzing network data and not for rendering the final product. This means that in order to visualize the network, the user has to make a lot of choices on graphic parameters,  %These packages further require to repeat all graphic parameters
...
To contrast, the packages we have written have one primary purpose: to provide a means of creating beautiful network visualizations within the popular \CRANpkg{ggplot2} package \citep{ggplot2}.
%
...
Thus, our focus here is not on adding to the important theory of graph drawing, \citep[cf.\ ][]{graphdrawing} but rather the implementation of current graph drawing capabilities in the \CRANpkg{ggplot2} framework. The \CRANpkg{ggplot2} package was designed as an implementation of the `grammar of graphics' proposed by \citet{wilkinson:1999}, and it has become extremely popular among R users.%
  %
...
applying the benefits of the grammar of graphics to network visualization.
...
actors connected in pairs by a set of %lines
relationships \citep{newman}. %Here, we refer to the lines as edges (or ties) and to the points as nodes (or vertices).
Here, we refer to the actors and relationships by their graph representation equivalents, vertices (or nodes) and edges, respectively. We note that our examples of visualization are all of one-mode networks, where all nodes are of the same kind and are able to connect to each other, however the graphical tools presented can be used to visualize %more general 
...
\item[Reproducibility:] Code and data used in the examples are available as part of the vignette in the \pkg{geomnet} package. %in the electronic supplement, which is hosted and kept maintained at \url{https://github.com/sctyner/ggnet-paper/tree/master/revision/supplementary\%20code}.
