...
davis$type <- factor(c(rep("Lady", nrow(net)), rep("Event", nrow(net))))
@
...
ggplot(data=davis) + 
  geom_net(
    aes(from_id=from, to_id=to, colour=type, 
        shape=type), 
    size = 15, label= TRUE, 
...
@
\caption{\label{fig:davis:geomnet}}
\end{figure}
