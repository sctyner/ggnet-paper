...
Many R packages already exist to manipulate network objects, such as \CRANpkg{igraph} by \citet{igraph}, \CRANpkg{sna} by \citet{sna}, and \CRANpkg{network} by \citet{network} (see also \citet{network.jss}). These packages, however, rely on idiosyncratic visualization syntaxes, which means that the user must acquire detailed knowledge of each one of these packages in order to build meaningful network visualizations with them. These packages further require to repeat all graphic parameters (such as colors, coordinates and sizes) in every network visualization, with no possibility to use precomputed layers of these parameters, or to map these parameters to network attributes.

% solution: ggplot2 as an implementation framework
In this article, our objective is to bring network plotting capabilities to the popular \CRANpkg{ggplot2} package \citep{ggplot2}, which was designed as an implementation of the `grammar of graphics' proposed by \citet{wilkinson:1999}, and which has become extremely popular among R users.%
  %
