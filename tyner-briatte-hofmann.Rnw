...
In its essence, a \dfn{network} is simply a set of %points
...
Throughout this paper, we also use the term \dfn{node} to refer to vertices, as well as the terms \dfn{ties} or \dfn{relationships} to refer to edges, depending on context. %We also note that our examples of visualization are all of one-mode networks, where all nodes are of the same kind and are able to connect to each other, however the graphical tools presented can be used to visualize %more general 
