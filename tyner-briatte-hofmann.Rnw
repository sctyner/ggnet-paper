...
rt <- list.files(path="runtimes") %>%
  lapply(function(x) paste0("runtimes/", x, "/", list.files(path=sprintf("runtimes/%s",x), pattern=".csv$"))) %>%
  unlist %>%
  lapply(read.csv, stringsAsFactors = FALSE) %>%
  bind_rows
rt$source <- rep(list.files(path="runtimes"), each=1000)
rtm <- melt(rt, id.var=c("network_size", "source", "iteration"))
qplot(data=rtm, network_size, value, colour=variable, facets=~source, alpha=I(0.1)) + 
