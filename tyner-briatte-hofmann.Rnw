...
\samp{geom\_nodes} is an alias to \samp{geom\_point}; \samp{geom\_edges} is an alias to
\samp{geom\_segment}; and the two other geoms for labels are just aliases to
\samp{geom\_text} (with some hacking for edge labels, because I draw a point
