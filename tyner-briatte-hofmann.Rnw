...
if (!require(ggnetwork, quietly = TRUE) ||
    packageVersion("ggnetwork") < "0.1.8") {
...
%\fb{Paragraph above now contains some information that were included in a footnote.}
...
Fortunately, R provides functionality that allows for an easy way of producing the required result: \code{merge} (in base R), \code{join} \citep[in \CRANpkg{plyr}][]{plyr} and \code{full\_join} \citep[in \CRANpkg{dplyr}][]{dplyr} can be used. In \code{merge} the argument \code{all} needs to be set to \code{TRUE}; in \code{join}, the parameters have to be set to \code{type = "full"} and \code{all = TRUE}. These operations are equivalent to a `full join' in SQL terminology.
...
At that stage, the \code{ggnetwork} function performs a left-join of that `augmented' edge list with the vertex-level attributes of the sender nodes, and further adds one self-loop per node, in order to ensure that every node is plotted even when their degree is zero---that is, even if the node is not connected to any other node of the network, and is therefore absent from the edge list. The data frame created by this process contains one row per edge as well as one additional row per node, and features all edge-level and vertex-level attributes initially present in the network.%
  %
  \footnote{One limitation of this process is that it requires some reserved variable names (\code{x}, \code{y}, \code{xend} and \code{yend}), which should not also be present as edge-level or vertex-level attributes (otherwise the function will simply break). Similarly, if an edge attribute and a vertex attribute have the same name, like  \samp{na}, \fb{which the \pkg{network} package defines as an attribute for both edges and vertices in order to flag missing data}, \code{ggnetwork} will rename them to \samp{na.x} (for the edge-level attribute) and \samp{na.y} (for the vertex-level attribute).} %

%% \hh{XXX -- \samp{"na"} is an unfortunate choice of name - it is easily confused with \code{NA}. I hope you're OK with my renaming this to ``name". }
%% FB: I'm fine with the renaming here, but I did not choose "na" by mistake: every object of class network carries a vertex attribute called 'na' and an edge attribute called 'na'. Both are used to flag missing (unobserved) data in network objects (see Butts' JSS article on the network package). Every data frame returned by ggnetwork therefore contains a column named na.x (indicating unobserved edges) and another one named na.y, indicating unobserved nodes.
%% HH: so my hunch with missing values was right, I just didn't interpret it right - I reverted the column names to `na', but added more details.
%% FB: thanks; reformulated.
...
%\fb{Paragraph above re-written to explain what 'panelized' meant in previous write-up.}
...
In this directed network, there are eight vertices and 27 edges.  The vertices represent the eight different blood types in humans that are most important for donation: the ABO blood types A, B, AB, and O, combined with the RhD positive (+) and negative (-) types. The edges are directed: a person whose blood type is that of a \emph{from} vertex can to donate blood to a person whose blood type is that of a corresponding \emph{to} vertex.  This network is shown in Figure~\ref{fig.cap:blood}. The code to produce each one of these results is given in Figure~\ref{tab:blood}. In this example every vertex has a self-reference, as blood between two people of matching ABO and RhD type can always be exchanged.  The \pkg{geomnet} approach shows these self-references as circles looping back to the vertex (this is controlled by using the parameter setting \code{selfies = TRUE}).

% \fb{May I recommend renaming \code{selfies} to \code{loops}, which is what self-ties are called in the \pkg{network} package?}
% \hh{What bugs me about the word \code{loops} is, that it is a misdenomer - loops are paths through a graph that lead back to the starting point (which plays havoc with all kinds of theoretical results, such as convergence). Self-references are therefore loops, but not all loops are self-references. Besides, I would like to think that it is pretty clear what is meant by selfies ... }
...
\begin{itemize}
  ...
  \item %
  In the \code{ggnet2} implementation, the darkness and thickness of the edges between employees in the same cluster are more pronounced than those of the edges between employees in different clusters. This is due to the fact that the email network does not feature edge weights: instead, every email between two employees is represented by a different edge, resulting in edge overplotting. Since the \code{edge.alpha} argument has been set to a value smaller than 1, multiple emails between two employees create darker, thicker edges between them. This effect is not visible in the layouts created by \code{geom\_net} and \code{ggnetwork} because both implementations draw a single segment \fb{for each directed edge}, regardless of how many of these edges exist in the network data. \hh{XXX single segment - that's not quite right - from-to and to-from are distinguished as multiple instances.} \fb{That's correct; reformulated.}
  ...
  \item %
  In the first two layouts, edges between employees who share the same employment type are given the color of that employment type, while edges between employees belonging to different types are plotted in grey. This feature is particularly useful to visualize the amount of within-group connectedness in a network. By contrast, in the last layout, edges are colored according to the sender's employment type, because the \code{ggnetwork} package does not support coloring edges as a function of node-level attributes.

	\item %
	  Finally, in the last two layouts, the \code{curvature} argument has been set to 0.05, resulting in slightly curved edges in both plots. This feature, which takes advantage of the \code{geom\_curve} geometry released in \pkg{ggplot2} 1.1.0, makes it possible to visualize which edges correspond to reciprocal connections; in an email communication network, as one might expect, most edges fall into that category.

\end{itemize}

\hh{XXX Last}
Last, it is interesting to observe that, in all of the layouts, the `Security' employee group gets a central role in the email network of the company. This observation holds regardless of the actual network layout used (remember that \code{ggnet2} and \code{ggnetwork} default to the Fruchterman-Reingold algorithm, while \code{geomnet} defaults to the Kamada-Kawai algorithm; both layouts are force-directed, but proceed differently to assign coordinates to the nodes). Interestingly, one of the executives only communicates with a single person in Facilities.\hh{XXX Last} Last, one of the IT employees is also frequently communicates with a Security employee.
%This pattern of heavy communication between employees of the same type is fairly unsurprising.
...
To \hh{introduce a temporal aspect}
%make this visualization even more interesting and informative
, we facet the network by day: each panel in Figure~\ref{fig:email_facet} shows email networks associated with each day of the week.
...
\begin{figure}[hbtp]
\caption{\label{tab:email_facet-geom_net} geomnet code for the  email network facetted by day as shown in figure~\ref{fig:email_facet}. }
...
\hh{Note that the main difference in the visualizations stems from whether \emph{one} layout is used across all panels (as for the \code{ggnetwork} example) or whether individual layouts are fit to each of the subsets (as for the \code{ggnet2} and the \code{geomnet} examples).  }
% Furthermore, and unlike \hh{the default setting in} \code{geomnet}, the same set of coordinates is assigned to all instances (i.e. to all rows) of the same node, which causes the position of each node to be identical in all facets of the plot.
\hh{In \code{geomnet} this is controlled via the argument \code{fiteach}. By default  \code{fiteach = TRUE}, but \code{fiteach = FALSE} results in all panels sharing the same layout.}
...
%\afterpage{\clearpage}
%
Figure~\ref{fig:email_facet} shows the facetted network. Through the facetting it becomes obvious that there are several days where one or more of the departments does not communicate with any of the other departments. There are only two days, day 13 and day 15, without any isolated department communications.
...
# see fig.11 for data preparation
...
\subsection{Character coappearances in \emph{Les Mis\'{e}rables}}
...
%% need this, otherwise "Floats lost" (TeX supports only a limited number of floats)
\clearpage
...
ggplot(data = ggnetwork(bikes.net),
  ...
  geom_nodetext(aes(label = vertex.names,
                ...
                vjust = -0.5) +
  scale_colour_manual("Metro station",
                      labels = c("FALSE", "TRUE"),
...
\includegraphics[width=.49\textwidth]{figure/bikes_ggnet2-1.pdf}
\includegraphics[width=.49\textwidth]{figure/bikes_ggnet2-2.pdf}
\caption{\label{fig:bikes_ggnet2} ggnetwork \fb{Problem: standardizing the coordinates to (0,1) distorts the aspect ratio}}
\end{figure}
...
\begin{figure}
\centering
<<compare, echo = FALSE, eval = TRUE, fig.height = 5, fig.width = 8, out.width = '0.5\\textwidth'>>=
