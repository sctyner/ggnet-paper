...
\midrule
Creating small multiples  & created separately, use \code{grid.arrange} from \pkg{gridExtra} & add g\code{group} argument to \code{fortify()} and use \code{facet\_*()} from \pkg{ggplot2} & use \code{by} argument in \code{ggnetwork()} and \code{facet\_*()} from \pkg{ggplot2}\\
\midrule
...
We think that users who are well-versed with networks will find \pkg{ggnet2} and \pkg{ggnetwork} more intuitive to use. These users might be looking to \pkg{ggplot2} to create high-quality visualizations that also have many additional visualization features open to them which familiar network packages do not.
