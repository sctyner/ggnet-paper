...
Note that edges between employees in the same cluster are  darker than edges between employees in different clusters. This is due to the \code{alpha} value of the edge: more emails between two employees lead to darker edges.  %The value is set to 0.25 in this example, so that four or more emails between two employees result in a completely opaque edge.
Additionally, \hh{in the \code{geom\_net} layout,} edges between employees within the same employment type are colored in the same color as the corresponding nodes, while edges between employees from different types are plotted using a shade of grey. \hh{In the \code{ggnetwork} layout, edges are colored according to the sender's employment type.}
\hh{Note that in these last two layouts \code{curvature} is set to 0.05, resulting in slightly curved edges. }

\hh{In  all of the layouts, Security gets a central role in the email network of the company. Interestingly, one of the executives only communicates with a single person in Facilities. One of the IT personnel is also less connected to the rest of IT group and communicates more often with Security.} %This pattern of heavy communication between employees of the same type is fairly unsurprising.
...
\hh{Note that the main difference in the visualizations stems from whether \emph{one} layout is used across all panels (as for the \code{ggnetwork} example) or whether individual layouts are fit to each of the subsets (as for the \code{ggnet2} and the \code{geom\_net} examples).  }
% Furthermore, and unlike \hh{the default setting in} \code{geom\_net}, the same set of coordinates is assigned to all instances (i.e. to all rows) of the same node, which causes the position of each node to be identical in all facets of the plot.
\hh{In \code{geom\_net} this is controlled via the argument \code{fiteach}. By default  \code{fiteach = TRUE}, but \code{fiteach = FALSE} results in all panels sharing the same layout.}
...
@
100 times and compared its run time to 100 runs each of the three visualisation approaches introduced in this paper. The results are shown in Figure~\ref{fig:timings}. The \pkg{ggplot2} framework provides a two to three-fold increase in speed on average.
}
\begin{figure}
\centering
...
Partly, because we had some `Annie' thing going on (I can do anything you can do), e.g.\ the edge coloring in ggnet2 and ggnetwork really forced some discussion on how to best implement edge coloring in geom\_net, which resulted  in the idea of the inside group/between group distinction using the \code{group} attribute. From one of your earlier comments, it sounded like that the whole geom idea of geom\_net spurred the ggnetwork approach in the first place ... Other things that made it into each one of our approaches are the gaps for the arrows and the curves.
...
\fb{I agree with the general observation here. And yes, ggnetwork was inspired by reading the code from geom\_net. I had never looked at the internals of ggplot2 before.}
