...
detailed knowledge of each one of these packages \sct{and their syntaxes} in order to build meaningful network visualizations with them. 
...
%How about: XXX  }
\hh{All of these packages are developed with a focus of analyzing network data and not for rendering the final product. This means that in order to visualize the network, the user has to make a lot of choices on graphic parameters,}  %These packages further require to repeat all graphic parameters
such as colors, coordinates and sizes, %in every network visualization, 
with no possibility to use precomputed defaults for these parameters, or to map these parameters to network attributes. 
\sct{To contrast, the packages we have written have one primary purpose: to provide a means of creating beautiful network visualizations within the popular \CRANpkg{ggplot2} package \citep{ggplot2}.}
%
...
\sct{Thus, our focus here is not on adding to the important theory of graph drawing, \citep[cf.\ ][]{graphdrawing} but rather the implementation of current graph drawing capabilities in the \CRANpkg{ggplot2} famework.} \sct{The \CRANpkg{ggplot2} package} was designed as an implementation of the `grammar of graphics' proposed by \citet{wilkinson:1999}, and it has become extremely popular among R users.%
  %
