...
The data shows the second quartal trips in 2015 taken with bikes from the bike share company Capital Bikeshare\footnote{\url{https://secure.capitalbikeshare.com/}}.
...
\fb{Suggestion: move 'comare to...' comment to text? footnote?}
\hh{XXX we can remove the link - but could you take a look at the map, please - the coordinates in ggnet2 and ggnetwork still do not look right. The map at \url{https://secure.capitalbikeshare.com/map/} shows the exact area covered
}
...
ggplot(data = ggnetwork(bikes.net, layout = coords),
       aes(x, y, xend = xend, yend = yend)) +
  geom_raster(data = metro_map,
              aes(x = long, y = lat, xend = NULL, yend = NULL),
              fill = metro_map$color, alpha = 0.75) +
  geom_edges(aes(size = n), color = "grey40") +
  geom_nodes(aes(color = factor(station)), size = 4) +
  geom_nodetext(aes(label = vertex.names, color = factor(station)),
                vjust = -0.5) +
...
The first test relies on random undirected networks in which the probability of a tie was set to 0.5. \hh{XXX We have been using `edge' in the paper - I'd stick to that and expand a bit on the explanation: `in which the probability for an edge between any two nodes was set to 0.5.' XXX This makes the networks pretty densele connected - none of our other networks is nearly that connected. How does the runtime vary with different tie probability?   } We generated 100 of these networks at different network sizes between 20 and 240 nodes, using increments of 20. We then computed the average plotting time at each network size, as shown in Figure~\ref{fig.cap:runtimes-random}. 
