...
\hh{XXX Francois, couple of questions
\begin{enumerate}
\item the code for ggnet2 is identical to the F-R layout. Is it possible to do a geographic layout in ggnet2?
\fb{The code is different: \code{mode = c("lon", "lat")}.}
\item I can't get the code for the geographic ggnetwork example to work. could you please take a look at it?
\fb{Same thing on my end, fixed by rescaling the coordinates of the raster image. Sorry for that.}
\end{enumerate}
}
<<geographic, echo=TRUE>>=
# overlay the bike sharing network on geographic map
...
# geographic placement ggnetwork
...
ggplot(data = ggnetwork(bikes.net, layout = coords),
       aes(x, y, xend = xend, yend = yend)) +
  geom_raster(data = map_01,
              aes(x = x, y = y, xend = NULL, yend = NULL),
...
\includegraphics[width=\textwidth]{figure/geographic-1.pdf}
\end{subfigure}
\begin{subfigure}[t]{.49\textwidth}
\caption{geomnet}
...
\section{Small consideration of speed}
...
qplot(data = timings, reorder(type, elapsed, median), elapsed, geom = "boxplot", fill = type, alpha = I(0.6)) +
...
The first test relies on random undirected networks in which the probability of an edge to exist between two nodes was set to 0.5. \hh{XXX We have been using `edge' in the paper - I'd stick to that and expand a bit on the explanation: `in which the probability for an edge between any two nodes was set to 0.5.' XXX This makes the networks pretty densele connected - none of our other networks is nearly that connected. How does the runtime vary with different tie probability?   } \fb{Let me run the same tests at p = 0.25 and tell you exactly. It sure affects the average plotting time, but the number of nodes should stay the most crucial parameter.} We generated 100 of these networks at different network sizes between 20 and 240 nodes, using increments of 20. We then computed the average plotting time at each network size, as shown in Figure~\ref{fig.cap:runtimes-random}.
\hh{XXX We need to get the comparison to be fair between the three methods. at the moment, the time in geomnet covers a) the extraction of the edgelist from an existing network, b) a re-calculation of the network in a K-K layout, and c) the actual plotting. All of the other routines do just the plotting.
XXX I'm not sure that we can actually fix this, if the network already exists.  I think this drives the message home again, that there are different motivations behind the three approaches. XXX }
\fb{The tests are now less biased: the network is generated outside of the system.time call for all functions. Also, all functions compute layouts internally, so I'm not sure what bias is left against geomnet.}
...
library(tidyr)
g = list.files("runtimes", pattern = "csv$", full.names = TRUE) %>%
...
\begin{enumerate}
\item Do you want to draw graph? if yes, you're off to a  good start, you're in the right place, keep reading. If no, sorry, but you can stop reading now.
