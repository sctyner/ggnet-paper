...
\par There are many kinds of networks, and networks are extensively studied across many disciplines.  \sct{Social network analysis is a prominent subfield of sociology, and the study of biological networks, such as protein-protein interaction networks or metabolic networks, is a notable subfield of biology \citet{prell2011social, junker2008analysis}. Thought these two disciplines and the many others that study networks are extremely differeny, they can all benefit from good network visualizations.}
...
mapping via the \pkg{ggmap} package by \citet{ggmap}, visualization of statistical models via the \pkg{ggfortify} package by \citet{ggfortify}, many helper functions in the \pkg{GGally} by \citep{ggally}, and \textit{Bioconductor} plotting via the \pkg{ggbio} by \citep{ggbio}, to name a few. Just as these examples have increased the utility of \pkg{ggplot2}, our additions aim to extend the benefits of the grammar of graphics to network visualization.

The most recent changes to \pkg{ggplot2} are aimed at
facilitating extension of \pkg{ggplot2} in form of additional \samp{geom} functions, which is exactly what we have done here.
...
Visualizing networks with a single layer as we do in \samp{geom\_net} is similar to the implementation of other, native \pkg{ggplot2} geoms, such as \samp{geom\_smooth}. When using \samp{geom\_smooth}, the user does not need to know about any of the internals of the loess function, and when using \samp{geom\_net}, they do not need to know about the internals of the layout algorithm. The user only needs to specify the essential elements necessary for calculating a layout. On the other hand, if the user is comfortable with network analysis, the whole apparatus of layout methods provided by the \pkg{sna} package is available to them through the parameters \samp{layout} and \samp{layout.par}. 
%
Network analysis is usally working with two sources of information: one data set consisting of a description of the actors, represented as the vertices in the network, and another data set detailing the relationship between these actors, the edge dataset. \sct{We assume the vertices are identified via an `id' column and that the edges are described by the vertices they connect via a `from' column and a `to' column whose entries are the vertex `ids'.} In order for this geometry to work, these two data sets need to be combined into a single data frame. For this, we are using the convention that all of the vertex information is merged into the edge data set using the `from' as the reference column. Generally, there will be some vertices that are sinks in the network because they only show up in the `to' column. We can easily accommodate for these by adding artificial edges in the data set that have missing information for the `to' column.
...
In this directed network, there are eight vertices and 27 edges.  The vertices represent the eight different blood types in humans that are most important for donation: the ABO blood types A, B, AB, and O, combined with the RhD positive (+) and negative (-) types. The edges are directed: a person whose blood type is that of a \emph{from} vertex can to donate blood to a person whose blood type is that of a corresponding \emph{to} vertex. \sct{XXX HH, should we leave the selfies in? I think they're fun and make a lot of sense in the blood ex. But we'd have to re-write this last sentence.} \hh{Let's leave them in for this example - I'm still fiddling with how to best place the loops. } In the example below, loops \hh{XXX these are self-references, loops are more general - in a directed graph any path that leads back to the same node is called a loop. we need to be careful with the language.} are removed because loops exist on every vertex in this example, as blood between two people of matching ABO and RhD type can always be exchanged.
...
<<email_facet_geom_net, fig.height=4.25, fig.width=8, echo=TRUE, out.width='\\textwidth'>>=
# source for geom_net
