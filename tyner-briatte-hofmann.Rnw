...
In this section, we demonstrate \sct{some of} the current capabilities of \code{ggnet2}, \code{geomnet}, and \code{ggnetwork} in a series of side by side examples. While the output is %similar
...
\hh{For the following examples we will be loading all three packages under comparison. In practice, only one of these packages would be needed to visualize a network in the \CRANpkg{ggplot2} framework:}
...
@
\end{subfigure}
\caption{\label{fig.cap:theme:c} Inheritance structure of \pkg{ggplot2} theme elements. This is a recreation of the graph found at \protect\url{http://docs.ggplot2.org/current/theme.html}.}
\end{figure}

%\afterpage{\clearpage}

The inheritance structure is plotted in figure \ref{fig.cap:theme}.  In these plots, it is easy to quickly determine  parent-child relationships, \sct{and to assess which theme elements are unrelated to all others.} Nodes with the most children are the \code{rect}, \code{text}, and \code{line} elements, so we made their labels larger in order to emphasize their importance. In each case, the label size is a function of the out degree of the vertices.
...
The code for the other two approaches is shown below: 
...
With a bit of data preparation as detailed below, we can visualize the graph as shown in Figure~\ref{fig:davis:geomnet}. }
...
@
\noindent
\hh{In order to distinguish between nodes from different types, we have to add an additional identifier element, so that we can tell the `first' woman $L1$ apart from the first event, $E1$.}
...
Figure~\ref{fig:bikes} shows four renderings of this data. The first is a geographically true representation of the area overlaid by lines between bike stations, the other three are networks drawn with \pkg{geomnet}, \pkg{ggnet2}, and \pkg{ggnetwork}, respectively. \sct{To plot the geographically correct bike network layout in \CRANpkg{geomnet}, we use the \samp{layout = NULL} option and provided the latitude and longitude coordinates of the bike stations from the company's data.}
\hh{We can also make use of the option \samp{layout = NULL} whenever we do not want to use an in-built layout algorithm but make use of a (user-defined) layout. In this case, the layout has to be rendered outside of the visualization and $x$ and $y$ coordinates have to be made available instead.}
...
\item[Reproducibility:] All the code and data used in the examples is available in the electronic supplement.
