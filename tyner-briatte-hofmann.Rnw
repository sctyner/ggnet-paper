...
\sct{To plot the geographically correct bike network layout in \CRANpkg{geomnet}, we use the \samp{layout = NULL} option and provide the latitude and longitude coordinates of the bike stations from the company's data.}
%
...
Because all three approaches result in the same picture, we only show one of these in Figure~\ref{fig:bikes}a. The code \sct{for creating the map} is given here:
