...
if (!file.exists("runtimes-protein-10.csv")) {
  ...
  write.csv(d, file = "runtimes-protein-10.csv", row.names = FALSE)
  
}

g = read.csv("runtimes-protein-10.csv", stringsAsFactors = FALSE) %>%
      gather(`Visualization approach`, time, -iteration) %>%
      group_by(`Visualization approach`) %>%
      summarise(mean_time = mean(time),
                q05 = quantile(time, 0.05),
                q95 = quantile(time, 0.95))

ggplot(g, aes(x = `Visualization approach`, y = mean_time,
              fill = `Visualization approach`)) +
  geom_boxplot() +
  coord_flip() +
  labs(x = "Average plotting time (seconds)\n") +
  guides(fill = FALSE) +
  theme_classic(18) +
